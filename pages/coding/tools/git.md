...
- [[#Useful Tips / Troubleshooting]]
	...
	- [[#Troubleshooting GitHub access]]
		...
		- [[#Troubleshooting error "WARNING REMOTE HOST IDENTIFICATION HAS CHANGED!"|Remote host identification changed]]
		- [[#API keys]]
	...
	- [[#Change your default commit message editor]]
...
```
# Useful links
- [official git docs](https://git-scm.com/about)
- [AutoTrader Git Training](https://github.atcloud.io/Autotrader/advanced-git-workshop) 
	- [slides are here](https://github.atcloud.io/pages/AutoTrader/advanced-git-workshop/#/)
	- lots of useful quizzes and exercises
	- might not be accessible outside of Autotrader!
- [git-it teaching app](https://github.com/jlord/git-it-electron) - A desktop (Mac, Windows and Linux) app that teaches you how to use Git and GitHub on the command line
...
- [ohshitgit.com](https://ohshitgit.com/) - A lovely little site by the wonderful [Katie Sylor-Miller](https://twitter.com/ksylor), designed to get you out of those horrible git messes you might get yourself in, using simple language.
	- The safe-for-work version: https://wizardzines.com/zines/dangit-git/
- Atlassian Git tutorial
	- Here: <https://www.atlassian.com/git/tutorials/>
- Another git tutorial: [Fracz tutorial](https://gitexercises.fracz.com/)
- Interactive tool to learn how to do git branching
	- [https://learngitbranching.js.org](https://learngitbranching.js.org)
- An interesting [blog post](https://alembic.com.au/blog/the-identity-of-a-git-commit-explained) about commit hashes
# GitHub Repos 
Sadly by necessity some of my repos are private. Those that are private are clearly marked. For those that are, please don't ask me to share the code, because I can't. They're listed here purely for my reference.
- [Git-Playground](https://github.com/claresudbery/Git-Playground)

# All Git Commands
### List of commands covered here
- [[#git init]]
- [[#git status]]
- [[#git add <filename >]]
- [[#git remote add]]
...
### git remote add
  ...
  - Eg: `git remote add remoteRepo https://github.com/try-git/try\_git.git`
...
### git commit
  - Normally…
      ...
      - \!\! You have to have the –m switch (followed by a commit
        message in quotes) – See [[vim#What to do if you get stuck in a VIM or VI editor|vim - What to do if you get stuck in a VIM or VI editor]] for what to do if you forget, and get stuck in a VIM window
...
### git reset
  ...
  - A note on flags
	  ...
	  - `reset --soft` leaves changes behind, staged and ready for commit
  - Can be used to …
  ...
  - Or... 
	  - Unstage AND REMOVE any staged changes since a *particular* commit
      - Like this: `git reset --hard db3989e` where `db3989e` is the commit hash
      - Or this: `git reset --hard HEAD~5` (see also [[#HEAD and FETCH_HEAD and HEAD~|HEAD~ syntax]])
  - Or...
	  - Unstage but do not remove changes from a particular commit
	  - `git reset --soft HEAD~5` ^head-squiggle
	  - If you follow this with `git commit`, this will effectively combine the last 5 commits into 1 commit. ^squash
		  - See also [[#HEAD and FETCH_HEAD and HEAD~]]
	  - This works even with changes in the working tree, which are left untouched, but such usage can lead to confusion.
  - Or…
...
- If you get stuck in the middle of a merge and want to abandon
    ...
    - Use this: `git merge abort`
        - If it says you need to commit changes first, you can just do
          `git stash` instead
	        - See separate [[#git stash|git stash section on this page]]
...
- If you get a merge conflict and you'd like to handle it by reverting
  - [Here is an example](https://github.com/claresudbery/Git-Playground/blob/master/Merging-examples.md) I manufactured when I was writing the O'Reilly trunk-based development (TBD) report
- Merging does not always create a new commit
	- If there have been no changes in the remote main/master since your branch diverged, it will essentially be a "fast forward" merge, and no new commit is needed
	- [More on that here](https://stackoverflow.com/questions/20222732/why-does-git-merge-sometimes-create-a-commit-sometimes-does-not)
...
### git stash
  ...
  - Stuff about applying stashes:
      ...
      - You don’t have to apply the stash to the branch it was
        originally saved on
### Git autostash
  - For `git stash`, [[#git stash|go here]]
  - `git autostash`
...
## git reflog
- [Tutorial Here](https://www.w3docs.com/learn-git/git-reflog.html)
- See also [this example](https://github.com/claresudbery/Git-Playground/blob/master/Merging-examples.md) I manufactured when I was writing the O'Reilly trunk-based development (TBD) report
...
- Locally:
  ...
  - `git push -u origin main`
	  - (The `-u` in this command is a short-hand for `--set-upstream`)
      ...
      - This one sort of did (apart from the permissions errors, which
        ...
        --recursive`
          ...
          - You can fiddle around with editing `.gitmodules` file and
            calling `git remote set-url origin https://github.xxx.com/Sysops/xxx.git`, but this didn’t
...
- Follow the link to regenerate personal access token (I think I was sent this in an email)
	- [This link here](https://github.atcloud.io/settings/tokens) (in GitHub Settings - Token - not in the repo but in your account settings)
...
## Setting up Git Command shortcuts (git alias)
  ...
  - If you add something or create duplicates by accident, you can
    directly edit the config file:
      ...
      - This will put you in a VIM editor – See [[vim#What to do if you get stuck in a VIM or VI editor|vim - What to do if you get stuck in a VIM or VI editor]]
## Changing git text colours used at command prompt
...
## Undoing all commits on your local copy of a shared branch 
  ...
  - Like this: `git reset --hard origin/master`
## Squashing commits
- This is where several commits are combined into one
- See [[#^squash|using reset --soft to squash commits]]
- You can also use `git rebase` and `git merge --squash` - [see discussion here](https://stackoverflow.com/questions/5189560/how-do-i-squash-my-last-n-commits-together)
## Checkout / Branch / Merge / Pull Confusions
...
## Vim Editor when no commit message
  - When you do a commit command and forget to add a message in Git
    Bash, you will end up in the VIM editor:
      - See [[vim#What to do if you get stuck in a VIM or VI editor|vim - What to do if you get stuck in a VIM or VI editor]]
      - You can also [[#Change your default commit message editor]]
## Change your default commit message editor
- We did [an exercise](https://github.atcloud.io/pages/AutoTrader/advanced-git-workshop/#/9) to change your editor to `nano` instead of `vi`
	- The exercise involved running these commands:
		- `echo 'export EDITOR=nano' >> ~/.zshrc`
		- `echo 'export VISUAL="$EDITOR"' >> ~/.zshrc`
		- `source ~/.zshrc`
	- Then I panicked cos I don't like nano, I prefer vi!
	- The exercise adds the following lines to the end of `~/.zshrc`:
		- `export EDITOR=nano`
		- `export VISUAL="$EDITOR"`
	- Those lines did not exist previously, so I guess the editor was set elsewhere
		- I could tell they weren't previously there by going to another command line tab where I hadn't sourced zshrc, and running `ECHO $EDITOR`, which returned zero results
	- To reverse the changes, you can just change `EDITOR=nano` to `EDITOR=vi`
		- NOTE: It's not enough remove or comment out those lines (and then source zshrc again), because the effect of those changes is external to zshrc, and will still be persisted. Because under the hood they've created env vars which still exist on your system even if you comment or remove those lines.
## Changesets vs snapshots
...
## Discard changes since the last commit that was pushed
  ...
  - See [[#git stash|separate detailed section]] for full details of `git stash`
...
## Git Flow
### These notes
  - …were originally headed "Git Playground" and written when I was at LateRooms, where I think I
...
## HEAD and FETCH_HEAD and HEAD~
  ...
  - `HEAD~1` is the commit before the current HEAD (so basically, HEAD minus 1)
	  - Similarly, `HEAD~5` is HEAD minus 5
	  - `HEAD~` is shorthand for `HEAD~1`
	  - See also [[#^head-squiggle|an example of HEAD~ syntax]]
## Git hooks
...
## What are commit hashes
- They contain so much info!
- [This blog post](https://alembic.com.au/blog/the-identity-of-a-git-commit-explained) is an interesting read on the topic
# Andy Grant brown bag - “Git - just like subversion, right?”
